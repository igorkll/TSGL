...
static tsgl_pos _mathObjectSize(tsgl_gui* object, bool height) {
    ...
    if (height) {
        return _localMath(object->format_height, 1, object->height, mHeight, minSide, maxSide, mWidth, mHeight);
    } else {
        return _localMath(object->format_width, 1, object->width, mWidth, minSide, maxSide, mWidth, mHeight);
    }
}
...
static void _math(tsgl_gui* object, tsgl_pos forceOffsetX, tsgl_pos forceOffsetY, bool force) {
    ...
    if (object->needMath || forceParentsMath) {
                    ...
                    if (x != object->tx || y != object->ty) {
                        if (object->draggable) {
                            ...
                            switch (object->tActionType) {
                                case 1 : {
                                    tsgl_pos new = TSGL_MATH_CLAMP(object->tdh - selY, minOffsetHeight, maxOffsetHeight);
                                    object->offsetY = object->tdy + (object->tdh - new);
                                    object->offsetHeight = new;
                                    break;
                                }

                                case 2:
                                    tsgl_pos new = TSGL_MATH_CLAMP(object->tdw - selX, minOffsetWidth, maxOffsetWidth);
                                    object->offsetX = object->tdx + (object->tdw - new);
                                    object->offsetWidth = new;
                                    break;

                                case 3:
                                    object->offsetHeight = TSGL_MATH_CLAMP(object->tdh + selY, minOffsetHeight, maxOffsetHeight);
                                    break;

                                case 4:
                                    object->offsetWidth = TSGL_MATH_CLAMP(object->tdw + selX, minOffsetWidth, maxOffsetWidth);
                                    break;
...
void tsgl_gui_setOffsetFromBorder(tsgl_gui* object, tsgl_gui_offsetFromBorder offsetFromBorder, tsgl_pos offsetX, tsgl_pos offsetY) {
    tsgl_gui_setPosFormat(object, tsgl_gui_absolute);
    ...
    switch (offsetFromBorder) {
        ...
        case tsgl_gui_offsetFromBorder_up_left:
            object->x = offsetX;
            object->y = offsetY;
            break;
