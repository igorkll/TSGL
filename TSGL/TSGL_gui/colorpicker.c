...
static void _fast_draw_callback(tsgl_gui* self) {
    ...
    self->colormode));
    TSGL_GUI_DRAW(self, fill, self->math_x + data->baseWidth, (self->math_y + pos) - 2, self->math_width - data->baseWidth, 5 - abs(data->oldHuePointerPos - oldpos), tsgl_color_raw(TSGL_WHITE, self->colormode));
