...
// allows you to draw sprites and rotate them when drawing. the tsgl_framebuffer_rotate method on sprite is ignored (because it changes the indexing)
void tsgl_framebuffer_push(tsgl_framebuffer* framebuffer, tsgl_pos x, tsgl_pos y, tsgl_sprite* sprite);
void tsgl_framebuffer_push_wtrans(tsgl_framebuffer* framebuffer, tsgl_pos x, tsgl_pos y, tsgl_sprite* sprite); // does not support transparency

// an extremely dumb and simplistic function. supports only the 565/666/888 color mode, does not support rotated framebuffers, not as a target, not as a renderable one. It does not support flip and rotation of sprites. Because it works fast. You also need to disable incomplete sending.
