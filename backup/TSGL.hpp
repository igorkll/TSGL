...
void set(tsgl_pos x, tsgl_pos y, tsgl_color color) { \
    set(x, y, tsgl_color_raw(color, colormode)); \
} \
 \
void line(tsgl_pos x1, tsgl_pos y1, tsgl_pos x2, tsgl_pos y2, tsgl_color color, tsgl_pos stroke) { \
    line(x1, y1, x2, y2, tsgl_color_raw(color, colormode), stroke); \
} \
 \
...
void fill(tsgl_pos x, tsgl_pos y, tsgl_pos width, tsgl_pos height, tsgl_color color) { \
    fill(x, y, width, height, tsgl_color_raw(color, colormode)); \
} \
 \
void rect(tsgl_pos x, tsgl_pos y, tsgl_pos width, tsgl_pos height, tsgl_color color, tsgl_pos stroke) { \
    rect(x, y, width, height, tsgl_color_raw(color, colormode), stroke); \
} \
 \
...
void clear(tsgl_color color) { \
    clear(tsgl_color_raw(color, colormode)); \
} \
 \
tsgl_color colorGet(tsgl_pos x, tsgl_pos y) { \
    return tsgl_color_uraw(get(x, y), colormode); \
} \
 \
...
uint32_t hexGet(tsgl_pos x, tsgl_pos y) { \
    return tsgl_color_toHex(colorGet(x, y)); \
}
...
class TSGL_Sprite {
    ...
    void init(tsgl_pos width, tsgl_pos height, tsgl_colormode colormode, int64_t caps) {
        if (caps == TSGL_NOBUFFER) {
            ESP_LOGE(TAG, "TSGL_Sprite cannot be initialized in TSGL_NOBUFFER mode");
            return;
        }
    ...
    void push(tsgl_pos x, tsgl_pos y, uint8_t rotation, TSGL_Sprite sprite) {
        tsgl_framebuffer_push(framebuffer, x, y, rotation, sprite.framebuffer);
    }
...
class TSGL_Display {
    void baseSpiInit(const tsgl_settings settings, int64_t caps, spi_host_device_t spihost, size_t freq, int8_t dc, int8_t cs, int8_t rst) {
        if (caps != TSGL_NOBUFFER) {
            framebuffer = (tsgl_framebuffer*)malloc(sizeof(tsgl_framebuffer));
    ...
    void init(const tsgl_settings settings, int64_t caps, spi_host_device_t spihost, size_t freq, int8_t dc, int8_t cs, int8_t rst) {
        ...
        baseSpiInit(settings, caps, spihost, freq, dc, cs, rst);
        this->caps = caps;
    }

    void init(const tsgl_settings settings, int64_t caps, spi_host_device_t spihost, size_t freq, int8_t mosi, int8_t miso, int8_t clk, int8_t dc, int8_t cs, int8_t rst) {
    ...
    TSGL_Sprite createSprite(tsgl_pos width, tsgl_pos height) {
        TSGL_Sprite sprite;
        sprite.init(width, height, colormode, caps);
        return sprite;
    }

    TSGL_Sprite createSprite(tsgl_pos width, tsgl_pos height, int64_t caps) {
        TSGL_Sprite sprite;
        sprite.init(width, height, colormode, caps);
        return sprite;
    }
    ...
    void push(tsgl_pos x, tsgl_pos y, uint8_t rotation, TSGL_Sprite sprite) {
        if (framebuffer == NULL) {
            tsgl_display_push(&display, x, y, rotation, sprite.framebuffer);
        } else {
            tsgl_framebuffer_push(framebuffer, x, y, rotation, sprite.framebuffer);
        }
    }
