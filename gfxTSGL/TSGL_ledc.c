...
#define LEDC2_DUTY_RES           LEDC_TIMER_8_BIT
#define LEDC2_FREQUENCY          25000
...
void tsgl_ledc_rawSet(tsgl_ledc* obj, uint8_t value) {
    if (obj->invert) {
        ledc_set_duty(LEDC_MODE, obj->channel, 255 - value);
    } else {
        ledc_set_duty(LEDC_MODE, obj->channel, value);
    }
    ledc_update_duty(LEDC_MODE, obj->channel);
}
